 ...
 * One pair of matching surrounding quotes is stripped from each item.
 */
export function parseFrontmatterList(raw: string | undefined): string[] | undefined {
	...
	return raw
		...
		.map((value) => stripListQuotes(value.trim()))
		.filter(Boolean);
}

/** Strip one pair of matching surrounding quotes from a list item (e.g. a quoted `- "read"` becomes `read`). */
function stripListQuotes(value: string): string {
...
export function parseFrontmatter(content: string): { frontmatter: Record<string, string>; body: string } {
	...
	for (const line of lines) {
		...
		// A list item at the SAME indent as its key is still part of that key's
		// block value (`key:` followed by same-indent `- item` lines is valid YAML
		// sequence syntax), as long as the block is not a literal/folded scalar
		// (those require deeper indentation than the key).
		const sameIndentListItem = indent === (currentIndent ?? 0) && !currentFolded && !currentLiteral && /^-\s/.test(trimmed);
		if (currentKey !== null && currentBlockLines !== null && (indent > (currentIndent ?? 0) || ((currentFolded || currentLiteral) && trimmed === "") || sameIndentListItem)) {
