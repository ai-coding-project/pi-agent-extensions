...
- **只读防护**（移植自 [@narumitw/pi-plan-mode-lite](https://github.com/narumiruna/pi-extensions) 的 tool-policy，MIT）：
  ...
  - bash / PowerShell 走**白名单 + fail-closed** 策略：无法安全解析的命令（重定向、反引号、子 shell、未闭合引号、`$`/`{` 展开、`\r`）一律拒绝；换行按 bash 语义处理（引号内是普通文本，引号外等价于 `;` 分隔符逐段校验）；未加引号的 glob（`*` `?` `[`）放行；白名单命令的危险参数（`sed -i`、`find -delete`、`sort -o` 等）单独拦截；git/gh 逐子命令、逐参数校验
